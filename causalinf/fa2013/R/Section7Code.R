...
# Now we have our data. This is where the problem would start if we were doing a study about this. So first, we will estimate the probability of treatment given our covariates (Age,Gender,Parents.Eaters).  If we have the right model of propensity scores, everything is fine.
...
pscore=glm(Treat ~ Age + Gender + Parents.Eaters + Age*Gender + Age*Parents.Eaters + Gender*Parents.Eaters, family= binomial(link=logit),data=data)$fitted.values
...
plot(seq(0,0.5, by=0.05),mean.of.estimates,ylim=c(0.25,0.35),xlab="Noise in Control",ylab="Expectation of Estimate",main="Bias Increases as Controls Become Noiser")
