...
# so, now let's add the pscore to our data set
data$pscore = pscore
data
...
# let's look at our matched data along with the weights
...
# let's let Match calculate the ATT for us
...
# notice that our ATT has changed... this is because we dropped observations.  This means our estimand isn't estimating what we think it is... but what is it measuring?
...
library(foreign)
data <- read.dta("./BalanceTest(n147).dta")
