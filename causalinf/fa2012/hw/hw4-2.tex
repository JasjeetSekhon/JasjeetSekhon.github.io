...
% URL provides David Lee's dataset: \url{http://sekhon.berkeley.edu/stuff1/LeeRDdata.zip}
...
%\item[c. ] Use the cross-validation procedure described in section to
 % calculate $h_{opt}$ for a trimmed subset of the data compiled by Caughey and Sekhon
  ...
  %(say $h = .01, .02, .03, ..., .3.)$ Using local linear regression, estimate the local average treatment effect and its associated standard error with your $h_{opt}$ calculated in part (a).
%\vspace{1em}
