...
\begin{document}
\begin{itemize}
% NOTE: THERE ARE PROBABLE ERRORS HERE:
%  \item[1)]
...
%    \begin{eqnarray*}
%      (r_{i0})_{i=1}^N & \independent & Z | X_{Z=1}\\	
%      P(Z = 1 | X_{Z=1}) &<& 1
%    \end{eqnarray*}
%    then conditioning by the propensity score
%    will make estimates of the ATT unbiased. \\ 
%    
%    To clear up notation:\\
%    \begin{eqnarray*}
%      Z & \equiv & (Z_i)_{i=1}^N\\
%      \{Z = 1\} &\equiv& \{i : Z_i = 1\}\\
%      X_{Z=1} & \equiv & (X_i)_{i \in \{Z = 1\}}\\
%      \mathcal I^1_p &\equiv & \{i : e(X_i) = p, Z_i = 1\} \\
%      X_{Z=1}\{p\} & \equiv & (X_i)_{i \in \mathcal I^1_p}
%    \end{eqnarray*}
%    If our population is large, with some of the population
%    assigned to treatment, and the rest assigned to control,
%    we can estimate the ATT by
%    \begin{align*}
%      E_{e(X_{Z=1})}[E[r_{i1}|Z_i=1, X_{Z=1}\{e(X_{Z=1})\}]-
%      E[r_{i0}|Z_{i}=0, X_{Z=1}\{e(X_{Z=1})\}]|e(X_{Z=1}) ].
%    \end{align*}
%    In words, we first fix a propensity score $p$.  
%    For both treatment and control responses, we compute
%    the expectation of all responses such that the
%    propensity score of their covariates is $p$.
%    We then take the expectation of the
%    difference of these expectations 
%    over the distribution of $e(X_{Z=1})$.\\
%    Note that the expectation
%    $$
%      E[r_{i0}|Z_{i}=0, X_{Z=1}\{e(X_{Z=1})\}]
%    $$
...
%      & E_{e(X_{Z=1})}[E[r_{i1}|Z_i=1, X_{Z=1}\{e(X_{Z=1})\}]-
%      E[r_{i0}|Z_{i}=0, X_{Z=1}\{e(X_{Z=1})\}]|e(X_{Z=1}) ] \\
%      = & E_{e(X_{Z=1})}[E[r_{i1}|Z_i=1, X_{Z=1}\{e(X_{Z=1})\},e(X_{Z=1})]]\\
%      &-E_{{e(X_{Z=1}})}[E[r_{i0}|Z_{i}=0, X_{Z=1}\{e(X_{Z=1})\},e(X_{Z=1})] ]\\
%      =& E_{e(X_{Z=1})}[E[r_{i1}|Z_i=1, X_{Z=1}\{e(X_{Z=1})\}]]-
%      E_{e(X_{Z=1})}[E[r_{i0}|Z_{i}=0, X_{Z=1}\{e(X_{Z=1})\}] ]\\
%      =& E_{X_{Z=1}}[E[r_{i1}|Z_i=1, X_{Z=1}]]-
%      E_{X_{Z=1}}[E[r_{i0}|Z_{i}=0, X_{Z=1}] ]
%    \end{align*}
%    The outer expectation in the last expression is now over
%    the distribution $X_{Z=1}$. \\
%    The third equality follows from 
%    $e(X_{Z=1})$ being a function of $X_{Z=1}$,
...
%      E_{X_{Z=1}}[E[r_{i0}|Z_{i}=0, X_{Z=1}] ] \\
%      =& E_{X_{Z=1}}[E[r_{i1}|Z_i=1, X_{Z=1}]]-
%      E_{X_{Z=1}}[E[r_{i0}|Z_{i}=1, X_{Z=1}] ] \\
%      = &E[r_{i1}|Z_i=1] - E[r_{i0}|Z_{i}=1] \\	  
%      = & E[r_{i1}-r_{i0}|Z_i=1] = \text{ATT}.
%    \end{align*}
%    Thus, when conditioning on the propensity score, the
%    estimate of the ATT is unbiased (asymptotically). \\
%    To obtain an unbiased ATE, we will need to 
%    modify the original assumptions to
%    \begin{align*}
%      (r_{i0})_{i=1}^N,(r_{i1},)_{i=1}^N  \independent  Z | X_{Z=1}\\	
%      0<P(Z = 1 | X_{Z=1}) < 1
%    \end{align*}
