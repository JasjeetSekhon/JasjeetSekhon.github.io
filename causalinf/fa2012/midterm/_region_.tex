\message{ !name(midterm_solutions12.tex)}
...
\message{ !name(midterm_solutions12.tex) !offset(-3) }
...
%The ATE estimate can be separable by groups:
%\begin{eqnarray}
%\nonumber        ATE = \frac{1}{4n}\left\{\sum_{i=1}^{4n}Y_i(1,M) -
 %         \sum_{i=1}^{4n} Y_i(0,M)\right\} \\
%= \frac{1}{4n}\left\{\sum_{i=1}^{2n}Y_i(1,1) - \sum_{i=1}^{2n}
 % Y_i(0,1) + \sum_{i=1}^{2n}Y_i(1,0) - \sum_{i=1}^{2n}
 % Y_i(0,0)\right\} 
%\end{eqnarray}
...
\nonumber   &=& \frac{1}{4n}\left\{\sum_{i\in\{1,M\}}Y_i(1,M) - \sum_{i\in\{0,M\}}
  Y_i(0,M)\right\} 
\end{eqnarray}
%which is composed of the averages of the observed outcomes for the
%four subpopulations.  
...
%$\hat{\beta}_2$ is 
%$$
%\hat{\beta}_2 =  \frac{1}{4n}\left\{\sum_{i=1}^{2n}Y_i(T,1) -
 %         \sum_{i=1}^{2n} Y_i(T,0)\right\} 
%$$
...
treated and the mean of $x$ for control reduces bias in an estimate of
the effect of $T$ on $Y$, so long as other moments of $x$ across
treated and control do not become more dissimilar.    

claim that bias is reduced
in an estimate of a treatment effect when balance is obtained in the
average moment of a confounder $x$, so long as differences in other
moments do not become dissimilar.  

joint distribution $X(1)$

      An affinely invariant matching method was used to
...
\message{ !name(midterm_solutions12.tex) !offset(-753) }
